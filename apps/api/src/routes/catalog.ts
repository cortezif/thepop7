...
export const catalogRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/sync", async (req, reply) => {
    const tenantId = req.auth!.tenantId;
    const { provider, trayCreds, blingCreds, omieCreds, vhsysCreds, connected } = await resolveErpCreds(tenantId);
    ...
    if (!connected) {
      const nome = provider === "bling" ? "a Bling" : provider === "omie" ? "a Omie" : provider === "vhsys" ? "a VHSYS" : "a Tray";
    ...
    const erp = buildErpForTenant({ trayCreds, blingCreds, omieCreds, vhsysCreds });
  ...
  app.get("/products/:id", async (req, reply) => {
    const id = (req.params as any).id;
    const { trayCreds, blingCreds, omieCreds, vhsysCreds } = await resolveErpCreds(req.auth!.tenantId);
    const erp = buildErpForTenant({ trayCreds, blingCreds, omieCreds, vhsysCreds });
