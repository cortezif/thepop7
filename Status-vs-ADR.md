...
| **016** | Handoff humano com inbox próprio | 🟢 | Inbox completo (lista/thread/reply/status), tool `escalar_para_humano`. **Co-piloto: botão "Sugerir (IA)" roda a Maya em modo read-only** (`POST /inbox/conversations/:id/suggest` → `suggestReply`), preenche o campo de resposta com sugestão editável sem persistir/mutar nada — **validado no navegador** | Aprendizado contínuo a partir das edições do atendente |
| **017** | Relatórios e financeiro embutidos | 🟡 | Dashboard com métricas reais, `/metrics/daily`. **Margem real**: `computeFinancials()` (order-service) sobre pedidos realizados — receita − COGS (`product.costBRL`) − taxa de gateway (PIX 0,99% / cartão 3,99%, sobrescrevível em `policies.gatewayFees`), frete pass-through; card no Dashboard com quebra + alerta de itens sem custo — **validado no navegador** (R$183,94 / 63,6%). **Funil de conversão**: `computeFunnel()` (conversa→pedido→pago→entregue com % por etapa) em `/metrics/daily.funnel`, barras no Dashboard — validado (5→1→1→1, 20%). **Export CSV**: `GET /orders/export.csv` (quebra financeira por pedido, BOM+`;`+`,` pro Excel pt-BR), botão na tela de Pedidos — validado. **Margem por produto no catálogo** (badge colorido por faixa: <40% âmbar / 40-60% neutro / >60% verde) — validado (Vestido 65%, Conjunto 80%) | Custo real de frete por transportadora |
...
| **022** | Cadeia de substitutos (fallback) | 🟢 | **Provider cascade LLM Claude→Groq→Ollama** + **failover de conectores**: `createFailover()` (Proxy genérico) encadeia provedor real → mock como último recurso pra ERP/logística/pagamento/NFe; cai pro próximo em erro recuperável (rede/timeout/5xx), propaga erro fatal — **validado** (503→mock, 400→propaga, OK→primário) | Health-check proativo (ping/circuit-breaker) — enhancement |
